...
{
	[Serializable]
	public class ToggleStyle : IStyle<Toggle>
	{
		private ToggleStyle(UI.Selectable.Transition transition) => m_Transition = transition;

		public ToggleStyle(ColorBlock color) : this(UI.Selectable.Transition.ColorTint) => m_Color = color;
		public ToggleStyle(SpriteState sprite) : this(UI.Selectable.Transition.SpriteSwap) => m_Sprite = sprite;
		public ToggleStyle(UI.AnimationTriggers animation) : this(UI.Selectable.Transition.Animation) => m_Animation = animation ?? new UI.AnimationTriggers();
		...
		{
			if (toggle == null)
				throw new NullReferenceException("Toggle reference does not refer to toggle object");
			toggle.transition = m_Transition;
			switch (m_Transition)
			{
				case UI.Selectable.Transition.ColorTint:
					toggle.colors = m_Color.Apply(toggle.setting);
					break;
				case UI.Selectable.Transition.SpriteSwap:
					toggle.spriteState = m_Sprite.Apply(toggle.setting);
					break;
				case UI.Selectable.Transition.Animation:
					toggle.animationTriggers = m_Animation;
					break;
